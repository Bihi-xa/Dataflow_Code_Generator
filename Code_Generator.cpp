	...
	for (int i = 1; i < argc; i++)
	{
		if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0)
		{
			std::cout << "\nUsage: %s [options]\n"
				...
				"   --abi=<stdc|stdc++|stdrust1|stdrust2>      Generate C, C++ or Rust code, default is C++.\n"
		...
		else if (strncmp(argv[i], "--abi=", 6) == 0)
		{
			...
			else if ((strat == "stdrust1") || (strat == "stdRust1"))
			{
				c->set_target_language(Target_Language::rust1);
				c->set_target_ABI(Target_ABI::stdrust1);
			}
			else if ((strat == "stdrust2") || (strat == "stdRust2"))
			{
				c->set_target_language(Target_Language::rust2);
				c->set_target_ABI(Target_ABI::stdrust2);
			}
			else
			{
...
int main(int argc, char* argv[])
{
	...
	{
		std::cout << "Generating Rust code. (Tokio + async/await)" << std::endl;
	}
	if (c->get_target_language() == Target_Language::rust2)
	{
		std::cout << "Generating Rust code. (Rayon + Crossbeam)" << std::endl;
	}
	std::cout << "Scheduling: ";
