...
public:
	...
	Actor_Diff_Data(
		...
		unsigned bound) : unused_actions{ a }, unused_in_channels{ in }, unused_out_channels{ out },
	...
	Mapping::Mapping_Data* map_data)
{
	...
	if ((c->get_target_language() == Target_Language::c) || (c->get_target_language() == Target_Language::cpp))
	{
	...
	else if (c->get_target_language() == Target_Language::rust1 || c->get_target_language() == Target_Language::rust2)
	{
	...
	*/
		...
		it != dpn->get_actor_instances().end(); ++it)
	{
		if ((*it)->get_composit_actor() == nullptr)
		{
			...
			if (actor_variants_map.contains(name))
			{
				...
				{
					if (std::equal(unused_actions.begin(), unused_actions.end(), known_it->unused_actions.begin()) && std::equal(unused_in_channels.begin(), unused_in_channels.end(), known_it->unused_in_channels.begin()) && std::equal(unused_out_channels.begin(), unused_out_channels.end(), known_it->unused_out_channels.begin()) && known_it->scheduling_loop_bound == (*it)->get_sched_loop_bound())
					{
						found = true;
					}
				}
	...
	for (auto it = actor_variants_map.begin(); it != actor_variants_map.end(); ++it)
	{
		...
		{
			size_t this_change = variant->unused_actions.size() + variant->unused_in_channels.size() + variant->unused_out_channels.size();
	...
	for (auto it = actor_variants_map.begin(); it != actor_variants_map.end(); ++it)
	{
		for (auto variant = it->second.begin(); variant != it->second.end(); ++variant)
		{
			...
			if (c->get_target_language() == Target_Language::rust1 || c->get_target_language() == Target_Language::rust2)
			{
	...
	if (c->get_target_language() == Target_Language::rust1 || c->get_target_language() == Target_Language::rust2)
	{
		// std::cout << "[DEBUG] Rust target selected.\n";
