	...
	std::string prefix)
{
	...
	ret.append(prefix + "done_flag: bool,\n");
	if (!fsm.empty())
	{
	...
	Actor_Conversion_Data &data)
{
		 ...
		 it != constructor_parameter_name_type_map.end(); ++it)
	{
		...
		if (used_in_channels.find(it->first) != used_in_channels.end() || c->get_target_language() == Target_Language::rust2)
		{
		 ...
		 it != actor->get_actions().end(); ++it)
	{
		...
		if (c->get_target_language() == Target_Language::rust1)
		{
			ret += convert_action_rust(*it, (*it)->get_action_buffer(), data,
									   true, false,
									   unused_in_channels, unused_out_channels, actor_var_map, action_param_read, prefix);
		}
		else
		{
			ret += convert_action_rust(*it, (*it)->get_action_buffer(), data,
	...
	std::string prefix)
{
		 ...
		 it != actor->get_actions().end(); ++it)
	{
		if ((*it)->is_init())
		{
			(*it)->get_action_buffer()->reset_buffer();
			ret += convert_action_rust(*it, (*it)->get_action_buffer(), data,
									   true, false, std::set<std::string>(), std::set<std::string>(), actor_var_map, action_param_read, prefix);
		}
	}
	...
	unsigned scheduling_loop_bound)
{
	...
	header_code.append(init_action_generation(actor, d, actor_var_map, action_param_read, "\t"));
	header_code.append("\n");
